...
// ── MENU (admin) — görsel / ekleme kolaylığı ─────────────────────

function MenuAdminImageBlock({
  previewUrl,
  uploading,
  urlDraft,
  onUrlDraft,
  ...
  return (
    <div className="admin-menu-img-block">
      <div
        className={`admin-menu-dropzone${drag ? ' admin-menu-dropzone--drag' : ''}`}
        onDragOver={e => {
          e.preventDefault()
          setDrag(true)
        }}
      ...
      >
        {previewUrl ? (
          <img src={previewUrl} alt="Önizleme" className="admin-menu-dropzone-preview" />
        ) : (
          <div className="admin-menu-dropzone-hint">
            <strong>Görsel yükle</strong>
            <span>Dosyayı sürükleyip bırakın veya aşağıdan seçin.</span>
          </div>
        )}
        <div className="admin-menu-dropzone-actions">
          <label className="admin-btn admin-btn-outline admin-menu-file-label">
            {uploading ? 'Yükleniyor…' : previewUrl ? 'Dosyayı değiştir' : 'Dosya seç'}
            <input type="file" accept="image/*" hidden disabled={uploading} onChange={e => { const f = e.target.files?.[0]; if (f) onFile(f); e.target.value = '' }} />
          </label>
          {previewUrl ? (
            <button type="button" className="admin-btn admin-btn-outline" onClick={onClear}>
              Görseli kaldır
            </button>
          ) : null}
        </div>
      </div>
      <div className="admin-menu-url-row">
        <input
        ...
        />
        <button type="button" className="admin-btn admin-btn-red" disabled={uploading} onClick={onApplyUrl}>
          Adresi uygula
        </button>
      </div>
    </div>
...
function MenuTab({ adminKey }: { adminKey: string }) {
  ...
  const [addImageUrlDraft, setAddImageUrlDraft] = useState('')
  ...
  useEffect(() => { load() }, [load])
  useEffect(() => {
    if (editItem) setEditImageUrlDraft('')
  }, [editItem?.id])
  ...
  const addCat = async () => {
    ...
    await post('/api/menu', { type: 'category', name: newCat.trim(), display_order: categories.length + 1 })
    setNewCat(''); load()
  }
  const deleteCat = async (id: string) => {
    const n = items.filter(i => i.category_id === id).length
    if (!confirm(n > 0 ? `Bu kategoride ${n} ürün var. Kategori ve tüm ürünler silinsin mi?` : 'Bu kategoriyi sil?')) return
  ...
  const addItem = async () => {
    ...
    try {
      const catItems = items.filter(i => i.category_id === form.category_id)
      const maxOrder = catItems.length ? Math.max(...catItems.map(i => i.display_order)) : 0
  ...
  const duplicateItem = async (item: Item) => {
    try {
      ...
      if (item.image_url?.trim()) payload.image_url = item.image_url.trim()
      await post('/api/menu', payload)
      load(); flash('Kopya oluşturuldu (pasif). Düzenleyip yayına alın.')
    } catch (e: unknown) { flash('Hata: ' + (e instanceof Error ? e.message : String(e))) }
  }
  ...
  const saveEdit = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        image_url: editItem.image_url?.trim() ? editItem.image_url.trim() : null,
      }
      await put(`/api/menu/${editItem.id}`, payload)
      setEditItem(null); setEditImageUrlDraft(''); load(); flash('Güncellendi ✓')
    } catch (e: unknown) { flash('Hata: ' + (e instanceof Error ? e.message : String(e))) }
  }
  ...
  const handleDropReorder = async (categoryId: string) => {
  ...
  const applyAddUrl = () => {
    const v = addImageUrlDraft.trim()
    if (!v) { flash('Önce bir görsel adresi yazın'); return }
    setForm(f => ({ ...f, image_url: v }))
    setAddImageUrlDraft('')
    flash('Görsel adresi uygulandı ✓')
  }
  const applyEditUrl = () => {
    if (!editItem) return
    const v = editImageUrlDraft.trim()
    if (!v) { flash('Önce bir görsel adresi yazın'); return }
    setEditItem({ ...editItem, image_url: v })
    setEditImageUrlDraft('')
    flash('Görsel adresi uygulandı ✓')
  }
    ...
    <div>
      <div className="admin-header">
        ...
        <div className="admin-page-sub">
          QR menü ve sitede listelenen ürünler. Satırları <strong>sürükleyip bırakarak</strong> sıralayın; görseli sürükleyip bırakabilir veya adres yapıştırabilirsiniz.
        </div>
      </div>

      <div className="admin-card">
        <div className="admin-card-title">Kategoriler</div>
        <p className="admin-menu-hint">Yeni kategori ekleyin; silmek için × kullanın (içindeki ürünler de silinir).</p>
        <div className="admin-menu-cat-add">
          <input
            className="admin-form"
            placeholder="Örn. Ana Yemekler, Çorbalar…"
            value={newCat}
            onChange={e => setNewCat(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && addCat()}
          />
          <button type="button" className="admin-btn admin-btn-red" onClick={addCat}>Kategori ekle</button>
        </div>
        <div className="admin-menu-cat-list">
          {sortedCategories.map(c => (
            <div key={c.id} className="admin-menu-cat-pill">
              <span>{c.name}</span>
              <button type="button" className="admin-menu-cat-remove" onClick={() => deleteCat(c.id)} aria-label="Kategoriyi sil">×</button>
            </div>
          ))}
        </div>
      </div>

      <div className="admin-card" ref={addFormRef} id="admin-menu-quick-add">
        <div className="admin-card-title">Yeni ürün ekle</div>
        {categories.length > 0 && (
          <div className="admin-menu-target-row">
            <span className="admin-menu-target-label">Hedef kategori</span>
            <div className="admin-menu-cat-chips">
              {sortedCategories.map(c => {
                ...
                return (
                  <button
                    key={c.id}
                    type="button"
                    className={`admin-menu-cat-chip${active ? ' admin-menu-cat-chip--active' : ''}`}
                    onClick={() => setForm(f => ({ ...f, category_id: c.id }))}
                  >
                    {c.name}
                    <span className="admin-menu-cat-chip-count">{n}</span>
                  </button>
                )
              })}
            </div>
        ...
        <div className="admin-form">
          <div className="admin-form-row">
            ...
            <div>
              <label>Fiyat</label>
              <input placeholder="Örn. 420 ₺" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} />
            </div>
          </div>
          <label>Ürün adı</label>
          <input placeholder="Örn. Kuzu tandır" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
          <label>Açıklama</label>
          <textarea placeholder="Kısa açıklama (isteğe bağlı)" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} style={{ minHeight: 88 }} />
          <label>Görsel</label>
          <MenuAdminImageBlock
            previewUrl={form.image_url}
            uploading={uploading}
            urlDraft={addImageUrlDraft}
            onUrlDraft={setAddImageUrlDraft}
            onApplyUrl={applyAddUrl}
            onFile={f => uploadImage(f, url => setForm(fm => ({ ...fm, image_url: url })))}
            onClear={() => { setForm(f => ({ ...f, image_url: '' })); setAddImageUrlDraft('') }}
          />
          ...
          <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            <button type="button" className="admin-btn admin-btn-red" onClick={addItem}>Ürünü kaydet</button>
      ...
      {categories.length === 0 && (
        <div className="admin-card"><div className="admin-empty">Önce en az bir kategori ekleyin.</div></div>
      )}

      {items.length === 0 && categories.length > 0 && (
        <div className="admin-card"><div className="admin-empty">Henüz ürün yok — yukarıdaki formdan ekleyin.</div></div>
      )}

      {sortedCategories.map(cat => {
        const catItems = items.filter(i => i.category_id === cat.id).sort((a, b) => a.display_order - b.display_order)
        return (
          <div key={cat.id} className="admin-card">
            <div className="admin-card-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
              <span>{cat.name}</span>
              <span style={{ fontWeight: 400, fontSize: 13, color: 'var(--muted)' }}>{catItems.length} ürün</span>
            </div>
            {catItems.length === 0 ? (
              <div className="admin-menu-empty-cat">
              ...
              <div className="admin-menu-table-wrap">
                <table className="admin-table admin-menu-table">
                  <thead>
                    <tr>
                      <th className="admin-menu-th-drag" aria-label="Sürükle" />
                      <th className="admin-menu-th-img">Görsel</th>
                      <th>Ürün</th>
                      <th>Fiyat</th>
                      <th>Durum</th>
                      <th>İşlemler</th>
                  ...
                  <tbody>
                    {catItems.map(item => (
                      <tr
                        ...
                        onDrop={() => handleDropReorder(cat.id)}
                        className="admin-menu-drag-row"
                        onDragEnd={() => { dragItem.current = null; dragOver.current = null }}
                      >
                        <td className="admin-menu-td-drag">⠿</td>
                        <td className="admin-menu-td-img">
                          {item.image_url ? (
                            <img src={item.image_url} alt="" className="admin-menu-thumb" />
                          ) : (
                            <div className="admin-menu-thumb admin-menu-thumb--empty" aria-hidden />
                          )}
                        </td>
                        <td>
                          ...
                          {item.is_featured ? <span className="badge badge-red" style={{ marginTop: 6, display: 'inline-block' }}>Öne çıkan</span> : null}
                        </td>
                        ...
                        <td>
                          <div className="admin-actions admin-menu-row-actions">
                            <button type="button" className="admin-btn admin-btn-outline" onClick={() => setEditItem(item)}>Düzenle</button>
                            <button type="button" className="admin-btn admin-btn-outline" onClick={() => duplicateItem(item)}>Kopyala</button>
                            <button type="button" className="admin-btn admin-btn-danger" onClick={() => deleteItem(item.id)}>Sil</button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      {editItem && (
        <div className="admin-menu-modal-overlay" role="presentation" onClick={e => { if (e.target === e.currentTarget) setEditItem(null) }}>
          <div className="admin-menu-modal" role="dialog" aria-modal="true" aria-labelledby="admin-menu-edit-title">
            <h3 id="admin-menu-edit-title" style={{ fontFamily: 'var(--serif)', fontSize: 22, marginBottom: 8 }}>Ürünü düzenle</h3>
            <p className="admin-menu-modal-lead">Kategori değiştirebilir, görseli kaldırabilir veya yayını açıp kapatabilirsiniz.</p>
            <div className="admin-form">
              <label>Kategori</label>
              <select value={editItem.category_id} onChange={e => setEditItem(i => i ? { ...i, category_id: e.target.value } : i)}>
                {sortedCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
              <label>Ürün adı</label>
              <input value={editItem.name} onChange={e => setEditItem(i => i ? { ...i, name: e.target.value } : i)} />
              <label>Açıklama</label>
              <textarea value={editItem.description || ''} onChange={e => setEditItem(i => i ? { ...i, description: e.target.value } : i)} style={{ minHeight: 88 }} />
              <label>Fiyat</label>
              <input value={editItem.price || ''} onChange={e => setEditItem(i => i ? { ...i, price: e.target.value } : i)} />
              <label>Görsel</label>
              <MenuAdminImageBlock
                previewUrl={editItem.image_url || ''}
                uploading={uploading}
                urlDraft={editImageUrlDraft}
                onUrlDraft={setEditImageUrlDraft}
                onApplyUrl={applyEditUrl}
                onFile={f => uploadImage(f, url => setEditItem(i => i ? { ...i, image_url: url } : i))}
                onClear={() => setEditItem(i => i ? { ...i, image_url: '' } : i)}
              />
              <label className="admin-toggle">
                <input type="checkbox" checked={editItem.is_available} onChange={e => setEditItem(i => i ? { ...i, is_available: e.target.checked } : i)} />
                <span>Menüde göster (QR ve sitede listelensin)</span>
              </label>
              <label className="admin-toggle">
            ...
            <div className="admin-menu-modal-footer">
              <button type="button" className="admin-btn admin-btn-red" onClick={saveEdit}>Kaydet</button>
              <button type="button" className="admin-btn admin-btn-outline" onClick={() => { setEditItem(null); setEditImageUrlDraft('') }}>İptal</button>
            </div>
          </div>
        </div>
      )}
...
function BlogTab({ adminKey }: { adminKey: string }) {
  ...
  const [filterType, setFilterType] = useState<'all' | 'tarif' | 'blog'>('all')
  const [sortBy, setSortBy] = useState<'date' | 'views'>('date')
  const [viewCounts, setViewCounts] = useState<Record<string, number>>({})

  const load = useCallback(async () => {
    const [data, settings, views] = await Promise.all([
      get('/api/posts'),
      fetch('/api/settings').then(r => r.json()),
      fetch('/api/post-views', { headers: { 'x-admin-key': adminKey } }).then(r => r.json()).catch(() => ({})),
    ])
        ...
        <div className="admin-card">
          <div style={{ display: 'flex', gap: 8, padding: '12px 16px', borderBottom: '1px solid var(--border)', flexWrap: 'wrap', alignItems: 'center' }}>
            <input
              ...
              style={{ flex: '1 1 240px', padding: '8px 12px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 14 }}
            />
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as 'all' | 'published' | 'draft')} style={{ padding: '8px 12px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 13 }}>
              <option value="all">Tümü</option>
              <option value="published">Yayında</option>
              <option value="draft">Taslak</option>
            </select>
            <select value={filterType} onChange={e => setFilterType(e.target.value as 'all' | 'tarif' | 'blog')} style={{ padding: '8px 12px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 13 }}>
              <option value="all">Tüm türler</option>
              <option value="tarif">🍽️ Tarif</option>
              <option value="blog">✍️ Blog</option>
            </select>
            <select value={sortBy} onChange={e => setSortBy(e.target.value as 'date' | 'views')} style={{ padding: '8px 12px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 13 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--muted)' }}>
              {(() => {
                ...
                const filtered = posts.filter(p => {
                  ...
                  if (filterType !== 'all' && (p.post_type || 'blog') !== filterType) return false
                  if (q && !p.title.toLocaleLowerCase('tr').includes(q) && !(p.excerpt || '').toLocaleLowerCase('tr').includes(q)) return false
                  return true
                })
                return `${filtered.length} / ${posts.length} kayıt`
              })()}
            </span>
          </div>
          {(() => {
            ...
            if (filteredPosts.length === 0) return <div className="admin-empty">Aramanıza uygun kayıt bulunamadı.</div>
            return (
                ...
                {filteredPosts.map(p => {
                  ...
                  <tr key={p.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap', textAlign: 'center' }}>
                      {views > 0
                        ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 13, fontWeight: 700, color: views >= 100 ? '#dc2626' : views >= 20 ? '#d97706' : '#374151' }}>
